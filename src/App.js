...
const generateQuizQuestions = (topic, dailyGoals, questionHistory, difficulty) => {
  ...
  if (questionHistory) {
    questionHistory.forEach(q => {
      const signature = q.question.replace(/[^a-zA-Z0-9]/g, '');
      if (!questionScores[signature]) {
        questionScores[signature] = { score: 0, count: 0 };
      }
      questionScores[signature].count++;
      if (q.isCorrect) {
        questionScores[signature].score += 1 / (q.timeTaken || 1);
      } else {
        questionScores[signature].score -= 1;
      }
    });
  }
